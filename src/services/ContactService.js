...
function getNextContactId(id) {
  let contactIdx = gContacts.findIndex(contact => contact._id === id)
  if (contactIdx === gContacts.length - 1) contactIdx = 0
  else contactIdx++
  return gContacts[contactIdx]._id
}

function getPrevContactId(id) {
  let contactIdx = gContacts.findIndex(contact => contact._id === id)
  if (contactIdx === 0) contactIdx = gContacts.length - 1
  else contactIdx--;
  return gContacts[contactIdx]._id
}
